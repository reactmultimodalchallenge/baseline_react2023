...
def parse_arg():
    ...
    parser.add_argument('--outdir', default="./results/test", type=str, help="result dir")
...
# Train
def test(args, model, val_loader, criterion, render):
    losses = AverageMeter()
    ...
    for batch_idx, (speaker_video_clip, speaker_audio_clip, speaker_emotion, listener_video_clip, _, listener_emotion, listener_3dmm, listener_references) in enumerate(tqdm(val_loader)):
        ...
        with torch.no_grad():
            ...
            if (batch_idx % 25) == 0:
                for bs in range(B):
                    render.rendering_for_fid(args.outdir, "val_b{}_ind{}".format(str(batch_idx + 1), str(bs + 1)),
    ...
    FRC = compute_FRC_mp(args, all_listener_emotion, listener_emotion, 'test')
    FRD = compute_FRD_mp(args, all_listener_emotion, listener_emotion, 'test')
...
def main(args):
    ...
    test_loss, rec_loss, kld_loss, FRC, FRD, FRDvs, FRVar, smse, TLCC = test(args, model, test_loader, criterion, render)
    print("test_loss: {:.5f}   test_rec_loss: {:.5f}  test_kld_loss: {:.5f} ".format(test_loss, rec_loss, kld_loss))
